...
|---|---|---|---|---|---|---|---|
| 1 | AWS_ACCESS_KEY_ID=REDACTED-AWS-KEY | .env | 15 | aws-access-token | IV-04 | CRITICAL | Full AWS cloud account compromise. Automated scanners detect AKIA pattern within minutes of public repo exposure |
| 2 | JWT_SECRET=REDACTED-ROTATED-SECRET | .env | 35 | jwt-secret | AV-07, IV-04 | CRITICAL | Attacker can forge JWT tokens and log in as any user including admin with no valid credentials |
| 3 | SESSION_SECRET=REDACTED-ROTATED-SECRET | .env | 40 | flask-secret-key | FV-03, IV-04 | CRITICAL | Attacker can forge Flask session cookies and hijack any active user session |
| 4 | AUTH_DB_PASSWORD=REDACTED-ROTATED-SECRET | .env | 20 | db-password-env | IV-01, IV-04 | CRITICAL | Direct read and write access to all user account data in the auth database |
| 5 | TRANSACTION_DB_PASSWORD=REDACTED-ROTATED-SECRET | .env | 26 | db-password-env | IV-01, IV-04 | CRITICAL | Direct read and write access to all transaction records |
| 6 | SONAR_TOKEN=REDACTED-examplesonarqubetoken... | .env | 44 | sonarqube-token | IV-04 | HIGH | Attacker can read code scan results, suppress findings, or disable security scanning entirely |
...
| # | Secret Found | File | Line | Rule ID | Vulnerability ID | Severity | Attack Risk |
|---|---|---|---|---|---|---|---|
| 7 | JWT_SECRET: REDACTED-ROTATED-SECRET | docker-compose.yml | 45 | jwt-secret | AV-07, IV-03 | CRITICAL | Same JWT secret as in .env — token forgery for any user account |
| 8 | SESSION_SECRET: REDACTED-ROTATED-SECRET | docker-compose.yml | 69 | flask-secret-key | FV-03, IV-03 | CRITICAL | Same session secret as in .env — full session hijack capability |
| 9 | POSTGRES_PASSWORD: REDACTED-ROTATED-SECRET | docker-compose.yml | 18 | db-password-env | IV-01, IV-03 | CRITICAL | Auth database password exposed — all user account data accessible |
| 10 | POSTGRES_PASSWORD: REDACTED-ROTATED-SECRET | docker-compose.yml | 30 | db-password-env | IV-01, IV-03 | CRITICAL | Transaction database password exposed — all transaction records accessible |
| 11 | DB_PASSWORD: REDACTED-ROTATED-SECRET | docker-compose.yml | 44 | db-password-env | IV-01, IV-03 | CRITICAL | Duplicate auth DB password in auth-service environment block |
| 12 | DB_PASSWORD: REDACTED-ROTATED-SECRET | docker-compose.yml | 58 | db-password-env | IV-01, IV-03 | CRITICAL | Duplicate transaction DB password in transaction-service environment block |
...
|---|---|---|---|---|---|---|---|
| 13 | SECRET_KEY = 'REDACTED-ROTATED-SECRET' | VULNERABILITIES.md | 19 | flask-secret-key | AV-07, IV-03 | MEDIUM | Documentation file containing the real secret value — Gitleaks correctly flags it regardless of context |
| 14 | SESSION_SECRET='REDACTED-ROTATED-SECRET' | VULNERABILITIES.md | 40 | flask-secret-key | FV-03, IV-03 | MEDIUM | Documentation file containing the real secret value — same issue as above |
...
| # | Secret Found | File | Line | Rule ID | Vulnerability ID | Severity | Attack Risk |
|---|---|---|---|---|---|---|---|
| 15 | JWT_SECRET: "REDACTED-ROTATED-SECRET" | infra/kubernetes/base/configmap.yaml | 9 | jwt-secret | AV-07, CK-09 | CRITICAL | Secrets stored in Kubernetes ConfigMap in plaintext — any pod in the cluster can read them |
| 16 | SESSION_SECRET: "REDACTED-ROTATED-SECRET" | infra/kubernetes/base/configmap.yaml | 10 | flask-secret-key | FV-03, CK-09 | CRITICAL | Session secret in ConfigMap — readable by any principal with namespace access |
| 17 | AUTH_DB_PASSWORD: "REDACTED-ROTATED-SECRET" | infra/kubernetes/base/configmap.yaml | 11 | db-password-env | IV-01, CK-09 | CRITICAL | Auth database password in ConfigMap — violates Kubernetes secrets best practice |
| 18 | TX_DB_PASSWORD: "REDACTED-ROTATED-SECRET" | infra/kubernetes/base/configmap.yaml | 12 | db-password-env | IV-01, CK-09 | CRITICAL | Transaction database password in ConfigMap — same violation |
...
| # | Secret Found | File | Line | Rule ID | Vulnerability ID | Severity | Attack Risk |
|---|---|---|---|---|---|---|---|
| 19 | password = "REDACTED-ROTATED-SECRET" | infra/terraform/main.tf | 66 | hashicorp-tf-password | IV-01, IV-08 | CRITICAL | Hardcoded database password in Terraform infrastructure code — anyone with repo access can access the RDS instance |
| 20 | db_password = "REDACTED-ROTATED-SECRET" | infra/terraform/main.tf | 66 | db-password-env | IV-01 | CRITICAL | Same finding detected by second rule — duplicate alert on same line |
